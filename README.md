...
- 앱은 두 개입니다. 화면을 그리는 frontend와 데이터를 돌려주는 backend이고, 서로 다른 이미지로 각각 따로 배포됩니다. 소스는 `app/frontend`와 `app/backend`에 있습니다. 브라우저에는 둘이 합쳐져 화면 하나로 보입니다.
- 2회차에 frontend를 올리고 4회차에 backend를 더합니다. 7회차 내내 같은 저장소를 씁니다.
- 회차마다 `k8s/`에 파일을 하나씩 더하거나, 이미 있는 파일에서 항목 하나를 다룹니다.
...
| 회차 | 주제 | 다루는 부분 | 가이드 |
|------|------|-------------|--------|
| 1 | 실행 | 부트스트랩, 클러스터 생성, 노드 확인 | [01-run.md](sessions/01-run.md) |
| 2 | 이해 | frontend 첫 배포, Pod, 선언적 모델 | [02-pod.md](sessions/02-pod.md) |
